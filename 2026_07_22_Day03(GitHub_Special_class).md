...
```markdown
## git clone과 pull
- 원격 저장소를 다른 컴퓨터로 가져오는 clone
 * 원격 저장소를 처음 통째로 복사할 때 사용
- 원격 저장소의 최신 병경 사항을 현재 로컬 저장소에 가져오는 pull
 * 이미 clone한 저장소에서 최신 변경 사항만 가져올 때 사용 
 
## 여러 컴퓨터나 여러 사람이 같은 저자송 사용할 경우 작업 시작 전 항상 pull을 실행
```
